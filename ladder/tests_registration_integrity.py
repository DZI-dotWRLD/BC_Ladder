...
from ladder.models import EmailNotificationDelivery, PlayerProfile, WorkflowEvent
...
class RegistrationIntegrityTests(TestCase):
    ...
    def test_registration_form_does_not_expose_invite_code(self):
        response = self.client.get(reverse("ladder:register"))

        self.assertEqual(response.status_code, 200)
        self.assertNotIn("invite_code", response.context["form"].fields)
        self.assertNotContains(response, "Invite code", html=False)
...
class PostgreSQLRegistrationRaceTests(TransactionTestCase):
    ...
    def test_competing_distinct_registrations_both_succeed(self):
        barrier = Barrier(2)
        original = PlayerRegistrationForm.clean_email
            ...
            try:
                return (
                    ...
                    .post(
                        ...
                        payload(f"player-{index}", f"player-{index}@example.com"),
                    )
                    .status_code
                )
            finally:
        ...
        self.assertEqual(get_user_model().objects.filter(username__startswith="player-").count(), 2)
