...
from ladder.models import AvailabilitySlot, LadderStanding, Match, MatchSuggestion, PlayerProfile, Team
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        with transaction.atomic():
            ...
            self._standings(mens_teams)
            self._standings(womens_teams)
        ...
        for index, team in enumerate(teams, start=1):
            LadderStanding.objects.get_or_create(
                ...
                defaults={
                    ...
                    "wins": max(0, index - 2),
                    "losses": 0 if index == 1 else 1,
                    "points": max(0, 12 - index * 2),
                },
            )
    ...
    def _availability(self, players, days_from_now, hour):
        ...
        for player in players:
            if AvailabilitySlot.objects.filter(player=player, starts_at=starts_at, ends_at=ends_at).exists():
                continue
